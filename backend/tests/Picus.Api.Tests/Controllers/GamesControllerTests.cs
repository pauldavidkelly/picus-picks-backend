...
public class GamesControllerTests
{
    ...
    [Fact]
    public async Task GetTeamGamesBySeason_WithExistingGames_ReturnsOkResult()
    {
        // Arrange
        var games = new List<GameDTO>
        {
            ...
            new()
            {
                ...
                ExternalGameId = "game2",
                Location = "Stadium 2",
                GameTime = DateTime.UtcNow.AddDays(7),
                PickDeadline = DateTime.UtcNow.AddDays(7),
                HomeTeamScore = null,
                AwayTeamScore = null,
                IsCompleted = false,
                Week = 18,
        ...
        _mockGameService.Setup(s => s.GetTeamGamesBySeasonAsync(1, 2023))
            .ReturnsAsync(games);

        // Act
        var result = await _controller.GetTeamGamesBySeason(1, 2023);
    ...
    [Fact]
    public async Task GetTeamGamesBySeason_WithNoGames_ReturnsEmptyList()
    {
        // Arrange
        _mockGameService.Setup(s => s.GetTeamGamesBySeasonAsync(1, 2023))
            .ReturnsAsync(new List<GameDTO>());
